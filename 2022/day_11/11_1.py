def main():
    ...
    false_decisions = [
        int(line.split("monkey ")[1]) for line in data if "false" in line
    ]
...
def play_round(
    ...
    tests: list,
    true_decisions: list,
    false_decisions: list,
    total_items_per_monkey: list,
):
    for monkey_id, monkey in enumerate(monkeys_items):
        ...
        for _ in range(len(monkey)):
            ...
            worry_level = (
                item * second_value if "*" in operation else item + second_value
            )
            worry_level //= 3
            # Next monkey id
            next_monkey_id = (
                true_decisions[monkey_id]
                if worry_level % tests[monkey_id] == 0
                else false_decisions[monkey_id]
            )
            # Move to monkey
